...
public class proxy : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string incomingProxyUrl = context.Request.Path;
        string uri = context.Request.Url.Query.Substring(1);

        //strip the token
        uri = RemoveQueryStringByKey(uri, "token");

        if (incomingProxyUrl.ToLower().Contains("/suggest"))
        {
            uri = uri.Replace("GeocodeServer=&", "GeocodeServer/suggest?");
        }
        if (incomingProxyUrl.ToLower().Contains("/findaddresscandidates"))
        {
            uri = uri.Replace("GeocodeServer=&", "GeocodeServer/findAddressCandidates?");
        }

        if (incomingProxyUrl.ToLower().Contains("/reversegeocode"))
        {
            string errorMsg = "Unauthorized request";
            sendErrorResponse(context.Response, null, errorMsg, System.Net.HttpStatusCode.Unauthorized);
            return;
        }
        if (incomingProxyUrl.ToLower().Contains("/geocode"))
        {
            string errorMsg = "Unauthorized request";
            sendErrorResponse(context.Response, null, errorMsg, System.Net.HttpStatusCode.Unauthorized);
            return;
    ...
    private string RemoveQueryStringByKey(string url, string key)
    {
       ...
       if (url.StartsWith("http%3a%2f%2f", StringComparison.InvariantCultureIgnoreCase) || url.StartsWith("https%3a%2f%2f", StringComparison.InvariantCultureIgnoreCase))
        ...
        var uri = new Uri(url.Replace("GeocodeServer&","GeocodeServer?"));
...
*/
    ...
    private string getNewTokenIfCredentialsAreSpecified(ServerUrl su, string reqUrl)
    {
        string token = "";
        string infoUrl = "";
        bool isPortalLogin = !String.IsNullOrEmpty(su.PortalUrl) && !String.IsNullOrEmpty(su.Password) && !String.IsNullOrEmpty(su.Username);
        bool isUserLogin = !String.IsNullOrEmpty(su.Username) && !String.IsNullOrEmpty(su.Password) && String.IsNullOrEmpty(su.PortalUrl);
        bool isAppLogin = !String.IsNullOrEmpty(su.ClientId) && !String.IsNullOrEmpty(su.ClientSecret) && String.IsNullOrEmpty(su.PortalUrl);
        if (isUserLogin || isAppLogin || isPortalLogin)
        {
            ...
            if (isPortalLogin)
            {
                ...
                if (su.PortalUrl != "")
                {
                    infoUrl = su.PortalUrl + "/sharing";
                }
                else if (reqUrl.ToLower().Contains("/rest/"))
                ...
                if (infoUrl != "")
                {
                    ...
                    log(TraceLevel.Info, " Querying security endpoint..." + infoUrl);
